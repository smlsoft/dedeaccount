...
class ReportTaxVatService {
     ...
     */
    async getJournal12columns(params) {
        try {
            // สร้าง URL ด้วย method ที่สร้างขึ้น
            const url = this.createApiUrl('apireport/journal12columns/');
     ...
     */
    async generateJournal12columnsPDF(params) {
        try {
            const url = this.createApiUrl('apireport/journal12columns/genPDF');
    ...
    async checkJobStatus(jobId, fileName) {
        try {
            ...
            const url = this.createApiUrl(`apireport/journal12columns/check/${jobId}/${fileName}`);
     ...
     * @param {string} fileName - ชื่อไฟล์
     */
    downloadJournal12columnsPDF(jobId, fileName) {
        if (!jobId || !fileName) {
            throw new Error('Job ID and file name are required');
        }

        try {
            const url = this.createApiUrl(`apireport/journal12columns/download/${jobId}/${fileName}`);
                ...
                try {
                    ...
                    try {
                        ...
                        if (status.completed) {
                            ...
                            this.downloadJournal12columnsPDF(jobId, fileName);
    ...
    async generateAndDownloadPDF(params) {
        try {
            ...
            const result = await this.generateJournal12columnsPDF(params);
