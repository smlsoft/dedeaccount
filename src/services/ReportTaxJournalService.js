...
class ReportTaxJournalService {
    ...
    async getTaxReport(params) {
        try {
            const url = new URL(`${this.baseUrl}apireport/journaltax/`);
    ...
    async generateTaxReportPDF(params) {
        try {
            const url = new URL(`${this.baseUrl}apireport/journaltax/genPDF`);
    ...
    async checkJobStatus(jobId, fileName) {
        try {
            const url = new URL(`${this.baseUrl}apireport/journaltax/check/${jobId}/${fileName}`);
            const response = await axios.get(url.toString());
            return {
    ...
    downloadTaxReportPDF(jobId, fileName) {
        const downloadUrl = `${this.baseUrl}apireport/journaltax/download/${jobId}/${fileName}`;
