...
onMounted(() => {
  getExpensesDailyList();
  storeApp.setPageTitle("รายการข้อมูลค่าใช้จ่ายอื่น ๆ");
  storeApp.setActivePage("daily");
  storeApp.setActiveChild("expenses_list");
});

function getExpensesDailyList() {
  loading.value = true;
  ExpensesDataService.getExpensesDailyList(
    limitPage.value,
    activePage.value,
...
function goForm() {
  router.push({ name: "expenses_form" });
}
...
function confirmDeleteDetail(data) {
  ...
  textContent.value = "ต้องการลบข้อมูลค่าใช้จ่ายอื่น ๆ เลขที่เอกสาร";
  confirmDeleteDialog.value = true;
}

function deleteDetail() {
  ExpensesDataService.deleteGLJournal(detail.value.guidfixed)
    .then((res) => {
      console.log(res);
      if (res.success) {
        getExpensesDailyList();
        toast.add({
          ...
          detail: "ลบเอกสารค่าใช้จ่ายอื่น ๆ สำเร็จ",
          life: 3000,
        });
          ...
          >
            <template #header>
              <div class="flex justify-content-between">
                <div>
                  <Button
                    label="เพิ่มข้อมูลค่าใช้จ่ายอื่น ๆ"
                    icon="pi pi-plus"
