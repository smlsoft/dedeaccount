...
import dataForm from "./form_input.vue";
    ...
    .then((res) => {
      if (res.success) {
        var newDataValue = res.data.map((x) => {
          // console.log(x);
          let addrZero =
            x.addressforbilling.address != null &&
            x.addressforbilling.address.length > 0
              ? x.addressforbilling.address[0]
              : "";
...
<template>
      ...
      <Splitter style="height: 100vh">
        <SplitterPanel :minSize="30">
          <dataList
            ...
            v-on:creditorCodeFocus="createCreditor"
            v-on:onRowSelect="onRowSelect"
            v-on:editCreditor="editCreditor"
            v-on:deleteCreditor="deleteCreditor"
            v-on:onPage="onPage"
        ...
        <SplitterPanel :minSize="30">
          <dataForm
            ...
            :creditorCodeFocus="creditorCodeFocus"
            :readMode="readMode"
            v-on:unFocusCreditorCode="unFocusCreditorCode"
            v-on:onSave="onSave"
          ></dataForm>
        </SplitterPanel>
      </Splitter>
  ...
  <DialogForm
    ...
    :textContent="'ต้องการบันทึกเจ้าหนี้'"
    :textContent2="form_model.name"
    v-on:close="confirmSaveDialog = false"
    v-on:confirm="confirmSave"
  ></DialogForm>
  <DialogForm
    :confirmDialog="confirmEditDialog"
    :textContent="'ต้องการแก้ไขเจ้าหนี้'"
    :textContent2="form_model.name"
