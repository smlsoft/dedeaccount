...
<template>
  <div class="surface-overlay relative">
    <div class="flex align-items-center justify-content-between lg:static" style="height: 65px">
      <div class="flex align-items-center bg-indigo-500 text-white h-full" style="height: 65px;width: 270px">
        ...
        <div class="ml-2 cursor-pointer" @click="openSelectShop()">{{ shopName }} <i class="pi pi-sort-alt ml-2"></i></div>
      </div>
        ...
        <ul
          ...
          <li class="border-top-1 surface-border lg:border-top-none">
            <a v-ripple
              ...
              <img src="@/assets/img/avatar-f-1.png" class="mr-3 lg:mr-0" style="width: 28px; height: 28px" />
            </a>
          </li>
        </ul>
      </div>
    ...
    <div id="navbar-7"
      ...
      <ul class="list-none p-0 m-0 flex flex-column lg:flex-row select-none">
        <li v-for="menu in menus" :key="menu.title" :class="menu.children.length > 0 ? 'lg:relative' : ''">
          <RouterLink v-if="menu.children.length == 0" :to="menu.to" :class="storeApp.pageActive == menu.name ? '' : ''"
            @click="
          ...
          <a v-if="menu.children.length > 0" v-ripple
            class="flex align-items-center cursor-pointer p-3 text-700 border-round hover:text-900 hover:surface-100 transition-duration-150 transition-colors p-ripple"
            style="border-radius: 12px" v-styleclass="{
              selector: '@next',
              enterClass: 'hidden',
          ...
          <ul v-if="menu.children.length > 0"
            ...
            <li v-for="child in menu.children" :key="child.title">
              <RouterLink :to="child.to" @click="
                storeApp.setActivePage(menu.name);
                storeApp.setActiveChild(child.name);
              " :class="storeApp.childActive == child.name ? '' : ''" v-ripple
        ...
        <li class="border-top-1 my-3 xl:my-0 xl:border-top-none border-indigo-400 xl:ml-auto">
  ...
  <Dialog header="เลือกร้านค้า" v-model:visible="displaySelectShop" ::breakpoints="{'960px': '75vw', '640px': '90vw'}"
    :style="{width: '70vw'}" position="top" :modal="true" :draggable="false">
    <div class="grid">
      <div class="col-12 md:col-4" v-for="shop in listShop" :key="shop">
        <div class="text-center border-1 surface-border border-round p-4">
          ...
          <Button label="เลือก" :disabled="shop.disable" class="p-button p-button-success w-full"
